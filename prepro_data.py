...
def build_data():
	...
	for datapath in filepaths:
		...
		# else:
		# 	sc = '/'
		# print(datapath.split(sc)[-1],len(index_split)-1,length_sentence)
...
def get_trainval_v1(test_radio,shuffle):
	data = []
	label = []
	train_data = []
	train_label = []
	...
	k = 3
	start_index = [x for x in range(len(data)) if data[x]=="\n"]
	start_index = [0] +start_index+[len(data)]


	for i in range(len(start_index)-k):
		subdata=data[start_index[i]:start_index[i+k]]
		sublabel=label[start_index[i]:start_index[i+k]]
		subdata = [x for x in subdata if x!="\n"]
		sublabel = [x for x in sublabel if x!="\n"]
		if sublabel ==[]:
			continue
		assert(len(subdata)==len(sublabel))
		train_data.append([wordsids.get(x,wordsids['UNK']) for x in subdata])
		train_label.append([classes.index(x) for x in sublabel])			
